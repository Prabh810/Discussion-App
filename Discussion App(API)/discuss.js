...
function deleteQuestionForm(deletedQue){

  getAllQuestions(function(questions){
        questions.forEach(function(que,index,array){
            if(deletedQue.title === que.title){
...
// update question
function updateQuestion(updatedQue){

  getAllQuestions(function(questions){
        var revisedQue = questions.map(function(que){
            if(updatedQue.title === que.title){
                return updatedQue;
            }
            return que;
        })
        // localStorage.setItem("questions",JSON.stringify(questions));
        var body = {
            data : JSON.stringify(revisedQue)
        }
        var request = new XMLHttpRequest();
        request.open("POST","https://storage.codequotient.com/data/save");
        request.setRequestHeader("Content-Type","application/json");
  ...
  //   return question;
  // })
  //   updateData(revisedQuestions,function(){
  //       console.log("updated")
  //   })
